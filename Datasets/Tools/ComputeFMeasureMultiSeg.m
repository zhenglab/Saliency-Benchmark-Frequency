%CVBIOUC% function [Results]=ComputeFMeasureMultiSeg(DBpath,SegResultsSubPath,SysType)
function ComputeFMeasureMultiSeg(DBpath,SysType) %CVBIOUC%
...
Lpath=dir(DBpath); %CVBIOUC%
%CVBIOUC% l=dir(DBpath);
%CVBIOUC% C=1;
%CVBIOUC% Results=zeros(length(Lpath),3);
...
for i=1:length(Lpath)
    ...
    %% END CVBIOUC
    %CVBIOUC% Hmask=GetHSeg(strcat(DBpath,Sep,Lpath(i),Sep,'Humen_seg',Sep));
    Hmask=GetHSeg(strcat(DBpath,Sep,Lpath(i,1).name,Sep,'human_seg',Sep),Lpath(i,1).name); %CVBIOUC%
    %% CVBIOUC
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%CVBIOUC% function [mask]=GetHSeg(Hpath)
function [mask]=GetHSeg(Hpath,GTname) %CVBIOUC%
% Get the human segmentation by considering the votes from each human
% subject
%CVBIOUC% l=dir(cell2mat(Hpath));
l=dir(Hpath); %CVBIOUC%
for k=1:length(l)
    if (l(k).isdir)
        continue;
    end;
    %CVBIOUC% im=im2double(imread(cell2mat(strcat(Hpath,l(k).name))));
    im=im2double(imread(strcat(Hpath,l(k).name))); %CVBIOUC%
    if (exist('mask','var'))
        %CVBIOUX% mask=mask+double((im(:,:,1)==1)&(im(:,:,2)==0));
        mask=mask+double((im(:,:,1)==1)&(im(:,:,2)==0))+double((im(:,:,3)==1)&(im(:,:,1)==0)); %CVBIOUX%
    else
        %CVBIOUX% mask=double((im(:,:,1)==1)&(im(:,:,2)==0));
        mask=double((im(:,:,1)==1)&(im(:,:,2)==0))+double((im(:,:,3)==1)&(im(:,:,1)==0)); %CVBIOUX%
    end;
end;
%% CVBIOUC 
...
% end;
%% END CVBIOUC
if max(mask(:))>1 %CVBIOUC%
    mask=mask>=2;
end %CVBIOUC%
imwrite(mask,strcat('./2obj_GroundTruth/',GTname,'.png')); %CVBIOUC%
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
